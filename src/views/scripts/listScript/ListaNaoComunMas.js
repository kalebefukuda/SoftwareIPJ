document.addEventListener('DOMContentLoaded', async () => {
    ...
    let data = [];
    const tbody = document.getElementById('listanaocomunmas');
    ...
    async function fetchData() {
        try {
            const response = await fetch('/api/lista-nao-comungantes-mas');
            ...
            console.error('Erro ao carregar lista de não comungantes masculinos:', error);
        }
    }
    ...
    document.getElementById('printBtn').addEventListener('click', () => {
        ...
        const title = "Lista de não comungantes - Masculino";
        ...
        while (currentDataIndex < data.length) {
            ...
            doc.autoTable({
                body: pageData.map(item => [
                    ...
                    item.numero_de_rol,
                    item.local_residencia
                ]),
                columns: [
                    { header: 'Nome', dataKey: 'nome' },
                    { header: 'Data de\nNascimento', dataKey: 'data_nascimento' },
                    { header: 'Rol', dataKey: 'numero_de_rol' },
                    { header: 'Local \nResidencia', dataKey: 'local_residencia' }
                ],
                ...
                headStyles: {
                    ...
                    fontStyle: 'bold',
                    halign: 'center',
                    cellPadding: { top: 3, right: 11, bottom: 3, left: 11 },
                },
                columnStyles: {
                    data_nascimento: { cellWidth: 60 } // Define `wrap` for column data_nascimento
                },
        ...
        doc.save('lista de não comungantes masculino.pdf');
    });
});
